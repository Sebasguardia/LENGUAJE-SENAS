...
const DashboardHome = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            // NO bloqueamos el hilo principal con await para que la UI se monte de inmediato
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-6 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col items-center gap-2">
                    <p className="dark:text-blue-400 text-blue-600 font-black text-[10px] uppercase tracking-[0.4em] animate-pulse">Sincronizando Red Neuronal</p>
                    <div className="flex gap-1">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="w-1 h-1 rounded-full bg-blue-500/40 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                        ))}
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 dark:bg-white/[0.02] bg-white backdrop-blur-3xl border dark:border-white/5 border-slate-200 p-8 rounded-[2.5rem] dark:shadow-2xl shadow-sm relative overflow-hidden">
                {/* Ambient glow */}
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h2 className="text-3xl font-black dark:text-white text-slate-900 tracking-tight leading-none mb-2">Panel Maestro IA</h2>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.5)]"></div>
                            <p className="dark:text-white/40 text-slate-500 text-sm font-bold uppercase tracking-widest">Sincronizado • Tiempo Real</p>
                        </div>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-3 sm:flex sm:items-center">
                    <div className="dark:bg-white/5 bg-slate-50 border dark:border-white/10 border-slate-200 px-6 py-4 rounded-2xl text-center">
                        <span className="block text-2xl font-black dark:text-white text-slate-900 leading-none mb-1">94%</span>
                        <span className="text-[10px] dark:text-white/30 text-slate-400 uppercase font-black tracking-widest">Salud Sistema</span>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <div key={i} className="dark:bg-white/[0.02] bg-white backdrop-blur-3xl border dark:border-white/5 border-slate-200 p-8 rounded-[2.5rem] dark:hover:border-white/10 hover:border-slate-300 dark:hover:bg-white/[0.04] hover:bg-slate-50 transition-all duration-300 group relative overflow-hidden shadow-sm dark:shadow-none">
                        <div className="absolute top-0 right-0 w-32 h-32 dark:bg-gradient-to-br dark:from-white/5 bg-gradient-to-br from-slate-100 to-transparent rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700"></div>
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-3xl font-black dark:text-white text-slate-900 tracking-tighter">{stat.value}</h3>
                                <p className="dark:text-white/80 text-slate-600 font-bold text-sm tracking-tight">{stat.label}</p>
                                <div className="flex items-center justify-between mt-4 pt-4 border-t dark:border-white/5 border-slate-100">
                                    <span className="dark:text-white/40 text-slate-400 text-[10px] uppercase font-black tracking-widest">{stat.subValue}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 dark:bg-white/[0.02] bg-white backdrop-blur-3xl rounded-[3rem] p-8 sm:p-10 border dark:border-white/5 border-slate-200 shadow-sm dark:shadow-2xl overflow-hidden relative group dark:hover:bg-white/[0.03] hover:bg-slate-50 transition-colors">
                    {/* Ambient glow */}
                    <div className="absolute -top-32 -left-32 w-64 h-64 bg-purple-500/10 blur-[60px] rounded-full pointer-events-none" />
                    <div className="flex items-center justify-between mb-10">
                        <div>
                            <h3 className="text-2xl font-black dark:text-white text-slate-900 tracking-tight">Densidad del Dataset por Módulo</h3>
                            <p className="dark:text-white/40 text-slate-500 text-sm font-medium mt-1">Muestras acumuladas para el entrenamiento neuronal</p>
                        </div>
                ...
                <div className="lg:col-span-4 dark:bg-white/[0.02] bg-white backdrop-blur-3xl rounded-[3rem] p-8 border dark:border-white/5 border-slate-200 dark:shadow-2xl shadow-sm flex flex-col h-full relative overflow-hidden dark:hover:bg-white/[0.03] hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-black dark:text-white text-slate-900 tracking-tight">Líderes de Aprendizaje</h3>
                    ...
                    <div className="flex-1 space-y-4">
                        {topStudents.length > 0 ? topStudents.map((user, i) => (
                            <div key={i} className="flex items-center gap-4 p-3 dark:bg-white/5 bg-slate-50 rounded-2xl border dark:border-white/5 border-slate-200 dark:hover:bg-white/10 hover:bg-slate-100 transition-colors">
                                ...
                                <div className="flex-1">
                                    <p className="dark:text-white text-slate-900 font-bold text-sm truncate">{user.name}</p>
                                    <p className="dark:text-white/40 text-slate-500 text-[10px] font-bold uppercase">Nivel {user.level}</p>
                                </div>
                                <div className="text-purple-500 dark:text-purple-400 font-black text-sm">{user.xp} XP</div>
                            </div>
                        )) : (
                            <div className="text-center dark:text-white/40 text-slate-500 text-sm py-10">Esperando datos de estudiantes...</div>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="dark:bg-white/[0.02] bg-white backdrop-blur-3xl border dark:border-white/5 border-slate-200 rounded-[2.5rem] p-10 dark:shadow-2xl shadow-sm overflow-hidden relative dark:hover:bg-white/[0.03] hover:bg-slate-50 transition-colors">
                <div className="absolute inset-0 dark:bg-grid-white/[0.02] bg-grid-slate-900/[0.02] bg-[length:20px_20px]"></div>

                <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start justify-between mb-8">
                    <div>
                        <div className="flex items-center gap-4 mb-3">
                            <div className="p-3 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
                                <CalendarDays className="text-emerald-500 dark:text-emerald-400" size={24} />
                            </div>
                            <h3 className="text-2xl font-black dark:text-white text-slate-900 tracking-tight">Actividad de Práctica Semanal</h3>
                        </div>
                        <p className="dark:text-white/40 text-slate-500 text-sm font-medium">Volumen de sesiones de práctica realizadas en los últimos 7 días</p>
                    </div>
                    <div className="bg-emerald-500/10 px-4 py-2 rounded-xl border border-emerald-500/20">
                        <span className="text-emerald-600 dark:text-emerald-400 text-xs font-black uppercase tracking-widest">+ Live Data</span>
                    </div>
                </div>
